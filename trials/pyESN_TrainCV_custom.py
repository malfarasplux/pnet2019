...
if mm:
    ...
    for i in range(n):
        i_pat = np.where(patient==i)[0]
        scaler.fit(feature_phys[i_pat,:])
        feature_phys[i_pat,:] = scaler.transform(feature_phys[i_pat,:])
    
    #Age
    feature_demog[:,0] = feature_demog[:,0] / 100.
    
    #HospAdmTime
    feature_demog[:,-2] = 1 - np.exp(-np.absolute(feature_demog[:,-2]))
    
    #ICULOS
    feature_demog[:,-1] = 1 - 1/(feature_demog[:,-1])
