...
dataset = "training"
path = "../../" + dataset +"/"
path = r"D:\Physionet Challenge\old_data"
kfold_split = 10
nan_to_neg = True
biased_regress = True
...
def sigmoid(x, exponent, bias):
    return 1/(1+np.exp(-exponent*x))-bias
...
def feedESN(features, neurons, mask, mask_bias, scale, mem, sigmoid_exponent, bias):
    ...
    for i in range(np.shape(features)[0]):
        ...
        ESN[i,:-1] = func(in_val, sigmoid_exponent, bias) #Apply transform
...
## Grid Search
# N_def_list = [20, 40, 50, 75, 100, 150, 200]
# scale_def_list = [.0001, .001, .005, .01, .1, 1, 10]
# mem_def_list = [.001, .01, .1, .13, .25, .5, 1]
# exponent_def_list = [1, .1, .5, 2, 5, 10, .01]
N_def_list = [200]
scale_def_list = [.1]
mem_def_list = [.001]
exponent_def_list = [.1]
bias_list = [-10, -1, -5, 0, .5, 1, 10]
bias_list = [.5]
...
for N in N_def_list:
    ...
    for scale in scale_def_list:
        for mem in mem_def_list:
            for sigmoid_exponent in exponent_def_list:
                for bias in bias_list:
                    ## Perform ESN feed
                    ESN = feedESN(feature_matrix, N, M, Mb, scale, mem, sigmoid_exponent, bias)
                    # del feature_matrix
                    ...
                    output_file = 'report_' + str(N) + '_' + str(scale) + '_' + str(mem) + '_' + str(sigmoid_exponent) + '_' + str(bias) + '.txt'
